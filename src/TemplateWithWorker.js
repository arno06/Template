...
class Template extends EventEmitter
{
    constructor(pIdTemplate, pContent = {})
    {
        ...
        let url = new URL(document.querySelector('script[src*="TemplateWithWorker.js"]').getAttribute("src"), window.location);
        this._rendererPath = url.searchParams.get('worker')??'Renderer-worker.js';
    }
    ...
    render(pParent)
    {
        ...
        worker.onmessage = (e)=>{
            switch(e.data.type){
                case Renderer.EVENT_EVALUATED:
                    this.evaluated(p, e.data.time, e.data.html);
                    worker.terminate();
                    break
            }
        };
        worker.postMessage({type:Renderer.EVENT_EVALUATE, content:this._content, templates:Template.$, id:this._id});
    }
